...
"""File name of the concat demuxer text file."""
...
def _args__parse():
    ...
    _parser = _ArgumentParser(
        ...
        description="Converts an image sequence (from ViSCA Recorder) to a video.",
...
def _concat_demuxer__generate():
    ...
    for index, file_name in enumerate(file_names__images):
        file_names__images[index] = _os_path.join(folder_name__images, file_name)
    # end for

    new_file_names__images = []

    for file_name in file_names__images:
        isfile = _os_path.isfile(file_name)
        basename = _os_path.basename(file_name)
        _, ext = _os_path.splitext(file_name)
        ext__matched = ext.lower() == ".png" or ext.lower() == ".jpg"

        if isfile and ext__matched:
            new_file_names__images.append(file_name)
        # end if
    # end for
    ...
    for index, file_name in enumerate(file_names__images):
        ...
        if basename__matched:
            ...
            time_s__curr = float(basename_split[1])
